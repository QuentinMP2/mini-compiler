...
let rec analyse_type_expression e =
  ...
  | AstTds.AppelFonction (info, le) -> begin
    ...
    | InfoFun(_, tr, argst) -> 
      ...
      let nle, tp = List.fold_left (fun acc x -> let x1,x2 = x in let acc1,acc2 = acc in acc1@[x1], acc2@[x2]) ([],[]) list_cpl_ne in
      if (est_compatible_list tp argst) then
        (AstType.AppelFonction(info, nle), tr)
      else
        raise (TypesParametresInattendus(tp, argst))
  ...
  | AstTds.Unaire (op,e1) -> begin
    ...
    match te1, op with
    | Rat,AstSyntax.Numerateur -> (AstType.Unaire(AstType.Numerateur, ne1), Int)
    | Rat,AstSyntax.Denominateur -> (AstType.Unaire(AstType.Denominateur, ne1), Int)
...
let rec analyse_type_instruction i = 
  ...
  | AstTds.Affectation (info, e) -> begin
    let ne, nt = analyse_type_expression e in
    match (!info) with 
    | InfoVar(_, typ, _, _) ->
      if (not (est_compatible nt typ)) then
        raise (TypeInattendu(nt, typ))
      else
  ...
  | AstTds.Conditionnelle (c,t,e) ->
    ...
    if (nctyp = Bool) then
      ...
      raise (TypeInattendu(nctyp, Bool))
...
let analyse_type_fonction (AstTds.Fonction(_,info,lp,li)) =
  ...
  let listI : info ref list = List.fold_left (
    (* On ajoute le type t à l'info i *)
    fun acc x -> 
      let t,i = x in 
      match (!i) with 
      | InfoVar(nom, _, depl, nomReg) -> let ni = ref (InfoVar(nom, t, depl, nomReg)) in acc@[ni]
      | _ -> failwith "erreur interne"  
    ) [] lp in 
