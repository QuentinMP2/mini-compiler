...
module VerifPlacement = struct
  ...
  let rec analyser_instruction i =
    match i with
    | Ast.AstPlacement.Declaration (info, _) | Ast.AstPlacement.StatiqueL (info, _) -> (
  ...
  let analyser (Ast.AstPlacement.Programme ((lvg, _), fonctions, (prog, _))) =
    ("VG", List.flatten (List.map analyser_instruction lvg))
